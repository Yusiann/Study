...
'''actions_dict 的输出结果为:
{87: 'Up', 65:'Left', 83:'Down', 68:'Right', 82:'Restart',81:'Exit',
119:'Up', 97:'Left', 115:'Down', 100:'Right',114: 'Restart', 113:'Exit'}'''

#阻塞+循环，直到获得用户有效输入才返回对应行为：
def get_user_action(keyboard): #用户输入处理
    char = 'N'
...
class GameField(object): #创建棋盘
    ...
    def spawn(self): # 随机生成一个2或者4
        #从100中取出一个随机数，如果这个随机数大于89，new_element 等于4，否则等于2
...
def main(stdscr): # 主程序
    ...
    def game(): # 游戏进行时的状态
        ...
        if action == 'Exit':
            return 'Exit'
        if game_field.move(action): # if成功移动了一步：
